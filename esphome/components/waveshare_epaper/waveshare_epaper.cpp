...
static const uint8_t LUT_SIZE_2P9_V2P1 = 159;

static const uint8_t FULL_UPDATE_LUT_2P9_V2P1[LUT_SIZE_2P9_V2P1] = {
...
static const uint8_t PARTIAL_UPDATE_LUT_2P9_V2P1[LUT_SIZE_2P9_V2P1] = {
...
void WaveshareEPaper2P9InV2P1::initialize() {
  if (this->hibernating_) {
    // wakeup
    reset_();
    this->wait_until_idle_();
...
void WaveshareEPaper2P9InV2P1::dump_config() {
  ESP_LOGCONFIG(TAG, "  Model: 2.9inV2.1");
  ESP_LOGCONFIG(TAG, "  Full Update Every: %u", this->full_update_every_);
  LOG_PIN("  Reset Pin: ", this->reset_pin_);
  LOG_PIN("  DC Pin: ", this->dc_pin_);
  LOG_PIN("  Busy Pin: ", this->busy_pin_);
...
void WaveshareEPaper2P9InV2P1::deep_sleep() {
  if (this->reset_pin_ != nullptr) {
    // power off
    this->command(0x22);
    this->data(0x83);
    ...
    this->command(0x10);
    this->data(0x01);
    this->hibernating_ = true;
  }
}

void WaveshareEPaper2P9InV2P1::display() {
  bool full_update = this->at_update_ == 0;
  bool prev_full_update = this->at_update_ == 1;
  ...
  if (this->full_update_every_ >= 1) {
    // init lut
    if (full_update != prev_full_update) {
      this->write_lut_(full_update ? FULL_UPDATE_LUT_2P9_V2P1 : PARTIAL_UPDATE_LUT_2P9_V2P1, LUT_SIZE_2P9_V2P1);
    }
    this->at_update_ = (this->at_update_ + 1) % this->full_update_every_;
  }
  ...
  // COMMAND DISPLAY UPDATE CONTROL 2
  this->command(0x22);
  this->data(full_update ? 0xF4 : 0xCC);
  ...
  // write as current as previous for partial update
  this->command(0x26);  // previous
  this->start_data_();
  this->write_array(this->buffer_, this->get_buffer_length_());
  this->end_data_();
